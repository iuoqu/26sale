...
async function callVision(apiKey, model, imageUrl, prompt, timeoutMs = 20000) {
...
async function getModel(apiKey, imageUrl, hintModel) {
  // If caller already knows a working model, verify it quickly
  if (hintModel) {
    try {
      await callVision(apiKey, hintModel, imageUrl, '这是什么？只回答一个词。', 5000);
  ...
  for (const m of MODELS) {
    try {
      await callVision(apiKey, m, imageUrl, '这是什么？只回答一个词。', 5000);
      cachedModel = m;
      return m;
    } catch {
      // 继续尝试下一个
...
export default async function handler(req, res) {
  ...
  const { imageUrl, type, productName, needGarment, needColor, model: hintModel } = req.body;
  if (!imageUrl) return res.status(400).json({ error: '缺少 imageUrl' });

  // 探测模型（首次请求时；后续请求通过 hintModel 跳过探测）
  const model = await getModel(apiKey, imageUrl, hintModel);
  ...
  try {
    if (needGarment && type === '衣') {
      ...
      result.garmentLabel = await callVision(apiKey, model, imageUrl, prompt);
    }

    if (needColor) {
      const prompt = `这件商品的主色调是什么？只输出一个颜色词，从以下选项选：${COLOR_OPTIONS}。不要解释。`;
      result.colorLabel = await callVision(apiKey, model, imageUrl, prompt);
    }
  } catch (e) {
