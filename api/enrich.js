...
async function getModel(apiKey, dataUrl) {
  if (cachedModel) return cachedModel;
  for (const m of MODELS) {
    try {
      await callVision(apiKey, m, dataUrl, '这是什么？只回答一个词。');
      cachedModel = m;
      return m;
    } catch {
      // 继续尝试下一个
...
export default async function handler(req, res) {
  ...
  const { dataUrl, type, productName, needGarment, needColor } = req.body;
  if (!dataUrl) return res.status(400).json({ error: '缺少 dataUrl' });

  // 探测模型（首次请求时）
  const model = await getModel(apiKey, dataUrl);
